...
<script>
  ...
  function buildImmunizationAccordion() {
    ...
    acc.addEventListener('click', function(e) {
      ...
      if (!h) return;
      var it = h.parentNode;
      acc.querySelectorAll('.acc-item').forEach(function(x){ if (x !== it) x.classList.remove('open') });
  ...
  function applySavedImmunization() {
    ...
    var map = {};
    saved.forEach(function(r) { map[(r.type || '') + '|' + (r.dose || '')] = r });
    acc.querySelectorAll('.dose-row').forEach(function(r) {
      ...
      setCell(cells[1], data.location);
      setCell(cells[2], data.date);
      setCell(cells[3], data.reaction);
      setCell(cells[4], data.remarks);
    });
  }
