...
<body class="has-mini-nav">
    <nav class="mini-nav">
        ...
        <button class="mini-nav-toggle" id="sidebarToggle">☰</button>
        <button class="sign-out" onclick="signOut()"><span>🚪</span><span class="nav-label">Sign Out</span></button>
    </nav>
...
// Build list of doctors to render (even if they have 0 appointments)
$doctorNames = [];
try {
    $stmt = $pdo->query("SELECT CONCAT('Dr. ', first_name, ' ', last_name) AS doctor_name FROM users WHERE role = 'doctor'");
    $doctorNames = $stmt->fetchAll(PDO::FETCH_COLUMN);
} catch (PDOException $e) {
    // Fallback: leave list empty, sections will still render for grouped keys
}
$allDoctors = array_unique(array_merge($doctorNames, array_keys($grouped)));
if (empty($allDoctors)) {
    $allDoctors = ['Unassigned'];
}

// Render each doctor's section (shows blank table if no appointments)
foreach ($allDoctors as $doctorName):
    $doctorAppointments = $grouped[$doctorName] ?? [];
?>
...
<?php endforeach; ?>
                ...
                <div class="calendar-section">
                    <div style="display: flex; justify-content: flex-end; align-items: center; margin-bottom: 8px;">
                        <div id="current-time" style="font-size: 1.2rem; color: #fff; background: linear-gradient(90deg, #F9DFDF 0%, #F5AFAF 100%); padding: 8px 22px; border-radius: 10px; font-weight: 500; box-shadow: 0 2px 8px rgba(245,175,175,0.15);"></div>
                    </div>
                            ...
                            .then(data => {
                                ...
                                data.data.forEach(appt => {
                                    ...
                                    fetch(approveAppointmentURL, {
                                        ...
                                        body: new URLSearchParams({ appointment_id: appt.appointment_id, status: 'approved' })
                                    })
                                    .then(res => res.json())
                                    .then(result => {
                                    ...
                                    rescheduleBtn.addEventListener('click', () => {
                                        ...
                                        const doctorNameEl = document.getElementById('rescheduleDoctorName');
                                        const doctorInfoEl = document.getElementById('rescheduleDoctorInfo');
                                        ...
                                        const docIdEl = document.getElementById('rescheduleDoctorId');

                                        apptIdEl.value = appt.appointment_id || '';
                                        docIdEl.value = appt.doctor_id || '';
                                        doctorNameEl.value = appt.doctor_name || 'Unassigned';
                                        doctorInfoEl.innerHTML = '';
                                        timeEl.innerHTML = '<option value="">Select Time</option>';
                                        dateEl.value = '';

                                        const parsed = parseScheduleText(appt.doctor_schedule || '');
                                        if (parsed) {
                                            const chip = document.createElement('div');
                                            chip.textContent = `${parsed.day} • ${parsed.start12} - ${parsed.end12}`;
                                            chip.className = 'schedule-chip';
                                            doctorInfoEl.appendChild(chip);
                                            ...
                                            warn.style.cssText = 'color:#ef4444;font-size:13px;';
                                            warn.textContent = 'No schedule set for this doctor.';
                                            doctorInfoEl.appendChild(warn);
                                        }
        ...
        <div id="rescheduleModal" class="modal-overlay" style="display: none;">
            <div class="modal-content" style="max-width: 560px; background: #ffffff; box-shadow: 0 16px 40px rgba(17,24,39,0.18); border-radius: 20px; padding: 0; overflow-y: auto; max-height: 90vh; border: 1px solid #e5e7eb;">
                <div style="background: linear-gradient(90deg,#7c3aed 0%, #b86cac 100%); padding: 20px 24px; border-radius: 20px 20px 0 0;">
                    <h2 style="color: #fff; font-size: 22px; font-weight: 700; margin: 0;">Reschedule Appointment</h2>
                </div>
                <form id="rescheduleForm" style="padding: 20px 24px;">
                    <input type="hidden" id="rescheduleAppointmentId" name="appointment_id" value="">
                    <input type="hidden" id="rescheduleDoctorId" name="doctor_id" value="">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px 20px; margin-bottom: 10px;">
                        <div style="display:flex;flex-direction:column;gap:8px;grid-column:1/-1">
                            <label style="color:#374151;font-weight:600">Doctor</label>
                            <input id="rescheduleDoctorName" name="doctor_name" readonly style="padding: 12px 14px; border-radius: 12px; border: 1.5px solid #e2e8f0; font-size: 15px; background: #f9fafb;">
                            <div id="rescheduleDoctorInfo" style="display:flex;flex-wrap:wrap;gap:8px;margin-top:6px"></div>
                        </div>
                        <div style="display:flex;flex-direction:column;gap:8px">
                            <label for="rescheduleDate" style="color:#374151;font-weight:600">Date</label>
                            <input type="date" id="rescheduleDate" name="date" required style="padding: 12px 14px; border-radius: 12px; border: 1.5px solid #e2e8f0; font-size: 15px; background: #fff;">
                        </div>
                        <div style="display:flex;flex-direction:column;gap:8px">
                            <label for="rescheduleTime" style="color:#374151;font-weight:600">Time</label>
                            <select id="rescheduleTime" name="time" required style="padding: 12px 14px; border-radius: 12px; border: 1.5px solid #e2e8f0; font-size: 15px; background: #fff;">
                                <option value="">Select Time</option>
                            </select>
                        </div>
                    </div>
                    <div style="display:flex; justify-content:flex-end; gap:10px; margin-top: 8px;">
                        <button type="button" class="btn-secondary" id="closeRescheduleModal" style="padding: 10px 18px; border-radius: 10px; font-size: 14px;">Cancel</button>
        ...
        function loadPatientList() {
            ...
            .then(data => {
                ...
                <td style="padding: 14px 18px;">
                    ...
                    <!-- View Profile Button -->
                    <button onclick="viewPatientProfile(${patient.patient_id})" class="action-btn-blue">
                    View Profile
                    </button>
                    <button
                    ...
                    onclick="
                        ...
                        loadPatientServices(${patient.patient_id}, '${patient.first_name} ${patient.last_name}');
                    "
                    >
                    View Services
                    </button>
    ...
    <div id="services-section" class="content-section" style="display:none; flex:1; background:#f9fafc; overflow-y:auto; padding:24px;">
        <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 28px;">
            <h2 class="page-title" id="patientServicesHeader" style="margin: 0; letter-spacing: .03em;">PATIENT SERVICES</h2>
            <button onclick="goBackToPatientList()" style="background:none; border:none; cursor:pointer; font-size:16px; color:#6b7280; display:flex; align-items:center; gap:8px; padding:8px 16px; border-radius:8px; transition:all 0.2s;">
        ...
        <div id="services-blank-state" class="services-container" style="display: flex; align-items: center; justify-content: center; min-height: 440px;">
            <div class="services-empty-state" style="max-width:470px; width:100%; text-align:center; margin:0 auto;">
                ...
                <div class="services-search">
                    <div style="position: relative; margin-bottom: 16px;">
                        <input 
                            ...
                            style="width: 100%; padding: 16px 20px; padding-left: 50px; border-radius: 16px; border: 2px solid #e5e7eb; font-size: 16px; outline: none; transition: all 0.3s ease; background: white; box-shadow: 0 4px 16px rgba(108,51,110,0.08); box-sizing: border-box;"
        ...
        function loadPatientServices(patientId, patientName) {
            ...
            let loadingIndicator = document.getElementById('services-loading-indicator');
            if (!loadingIndicator) {
                loadingIndicator = document.createElement('div');
                loadingIndicator.id = 'services-loading-indicator';
                loadingIndicator.style.textAlign = 'center';
                loadingIndicator.style.padding = '40px';
                loadingIndicator.style.color = '#6b7280';
                loadingIndicator.style.fontSize = '16px';
                loadingIndicator.innerHTML = 'Loading services...';
                section.insertBefore(loadingIndicator, servicesContent);
            }
            ...
            fetch(`http://localhost/JAM_LYINGIN/auth/action/clerk/clerk_get_services_by_patient.php?patient_id=${patientId}`)
                ...
                .then(data => {
                    ...
                    // Remove any previous service table
                    const oldTable = document.getElementById('patient-services-table');
                    if (oldTable) oldTable.remove();

                    // Always show dashboard, hide blank state
                    blankState.style.display = 'none';
                    ...
                    // Update hidden input for Add Service modal
                    const hiddenIdInput = document.getElementById('patient_id_hidden');
                    if (hiddenIdInput) hiddenIdInput.value = patientId;

                    const tableBody = document.getElementById('services-table-body');

                    if (data.status === 'success' && Array.isArray(data.data) && data.data.length > 0) {
                        // Populate services table
                        tableBody.innerHTML = data.data.map(service => `
                            <tr style="background: #fff; border-bottom: 1px solid #f3e6f7; transition: all 0.2s ease;" onmouseover="this.style.background='#faf5ff'" onmouseout="this.style.background='#fff'">
                                <td style="padding: 16px 18px;">${service.service_date}</td>
                                <td style="padding: 16px 18px;">${service.service_type}</td>
                                <td style="padding: 16px 18px; font-weight: 600; color: #6c336e;">₱${parseFloat(service.service_amount || 0).toFixed(2)}</td>
                                <td style="padding: 16px 18px; color: #666;">${(service.notes || '—').replace(/\n/g, '<br>')}</td>
                            </tr>
                        `).join('');
                        ...
                        if (data.status !== 'success') {
                            console.warn(`⚠️ Error loading services for patient ${patientId}:`, data.message);
                        }
                    }
                })
                .catch(err => {
                    ...
                    tableBody.innerHTML = `
                        <tr style="background: #fff; border-bottom: 1px solid #f3e6f7;">
                            <td colspan="4" style="padding: 40px 18px; text-align: center; color: #b91c1c;">
                                <div style="font-size: 16px; margin-bottom: 8px;">⚠️</div>
                                <p style="margin: 0; font-size: 14px;">Failed to load services. Please try again.</p>
                            </td>
                        </tr>
                    `;
                });
        ...
        function goBackToPatientList() {
            document.getElementById('services-section').style.display = 'none';
            document.getElementById('services-blank-state').style.display = 'flex';
        ...
        <div id="services-content" style="display:none; width:100%; margin:0; padding:24px;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; padding-bottom: 20px; border-bottom: 2px solid #f3e6f7; flex-wrap: wrap; gap: 16px;">
            ...
            <div style="overflow-x: auto;">
                <table id="services-table" style="width: 100%; border-collapse: separate; border-spacing: 0; min-width: 700px; border-radius: 18px; overflow: hidden; box-shadow: 0 4px 16px rgba(108,51,110,0.07); background:#fff;">
                    <thead>
                        <tr style="background: linear-gradient(90deg, rgb(251, 137, 184) 0%, #764ba2 100%); color: #fff;">
...
<div id="billingDashboard" class="content-section" style="display:none; flex:1; background:#f9fafc; overflow-y:auto; padding:24px;">
  ...
  <div style="padding:32px 0; margin:0 auto;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 32px;">
      ...
      <button onclick="document.getElementById('billingDashboard').style.display='none'; document.getElementById('patient-list-section').style.display='block';" style="background:none; border:none; cursor:pointer; font-size:16px; color:#6b7280; display:flex; align-items:center; gap:8px; padding:8px 16px; border-radius:8px; transition:all 0.2s;">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M19 12H5M12 19l-7-7 7-7"/>
          </svg>
          Back to Patients
    ...
    <div id="addBillModal" class="modal-overlay">
      <div class="modal-content">
        <div class="modal-header">
             <h2>Add Billing Transaction</h2>
             <button onclick="document.getElementById('addBillModal').style.display='none'" class="modal-close">&times;</button>
        </div>
        <div class="modal-form">
            <div class="form-row" style="grid-template-columns: 1fr 1fr;">
                <div class="form-group">
                    <label for="billDescription">Description</label>
                    <input type="text" id="billDescription" placeholder="Description">
                </div>
                <div class="form-group">
            ...
            <div class="form-row" style="grid-template-columns: 1fr 1fr;">
                <div class="form-group">
                    <label for="billServiceId">Service</label>
                    <select id="billServiceId">
                        <option value="">Select Service</option>
            ...
            <button onclick="submitBill()" class="btn-primary" style="width:100%; padding:12px; border-radius:12px;">Submit</button>
        </div>
      </div>
    </div>
    ...
    <div style="background:#fff; border-radius:18px; box-shadow:0 2px 12px rgba(44,62,80,0.06); padding:28px 32px; margin-bottom:32px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:18px;">
        ...
        <div style="display: flex; gap: 10px; align-items: center;">
             <input type="text" id="transactionDateSearch" placeholder="Search Date..." style="padding:10px 16px; border:1px solid #e2e8f0; border-radius:8px; font-size:14px; outline:none;" oninput="filterTransactions()">
      ...
      if (data.status === 'success') {
        ...
        data.data.forEach(tx => {
          ...
          row.innerHTML = `
            ...
            <td style="padding:8px 10px;">${escapeHtml(tx.description.toUpperCase())}</td>
            <td style="padding:8px 10px;">₱${parseFloat(tx.amount).toFixed(2)}</td>
            <td style="padding:8px 10px;">${tx.transaction_type.toUpperCase()}</td>
          `;
...
function filterTransactions() {
  const input = document.getElementById('transactionDateSearch');
  const filter = input.value.toUpperCase();
  const table = document.getElementById('billingTransactionTable');
...
document.getElementById('sidebarToggle').addEventListener('click', function() {
  const sb = document.querySelector('.mini-nav');
  const bodyEl = document.body;
  sb.classList.toggle('collapsed');
  bodyEl.classList.toggle('nav-collapsed');
});

// Hash-based routing: open Patients tab when URL ends with #patients
(function() {
  const hash = (window.location.hash || '').toLowerCase();
  const tabParam = new URLSearchParams(window.location.search).get('tab');
  if (hash === '#patients' || (tabParam && tabParam.toLowerCase() === 'patients')) {
    const mainContent = document.querySelector('.main-content > .content-section');
    const patientList = document.getElementById('patient-list-section');
    const servicesSection = document.getElementById('services-section');
    const billingDashboard = document.getElementById('billingDashboard');
    const settingsSection = document.getElementById('settings-section');
    if (mainContent && patientList && servicesSection && billingDashboard && settingsSection) {
      mainContent.style.display = 'none';
      servicesSection.style.display = 'none';
      billingDashboard.style.display = 'none';
      settingsSection.style.display = 'none';
      ...
      document.querySelectorAll('.nav-item').forEach(nav => {
        if (nav.textContent.trim().includes('Patients')) {
        ...
        (function setupRescheduleModal(){
            ...
            form.addEventListener('submit', function(e){
                ...
                const apptId = apptIdEl.value;
                if (!dateStr || !timeStr) { alert('Please select date and time.'); return; }

                fetch(rescheduleAppointmentURL, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: new URLSearchParams({ appointment_id: apptId, date: dateStr, time: timeStr })
                })
                .then(res => res.json())
                .then(data => {
                    alert(data.message || (data.status === 'success' ? 'Rescheduled' : 'Failed to reschedule'));
                    if (data.status === 'success') {
                        modal.style.display = 'none';
                        // Refresh pending appointments
                        const container = document.getElementById('pending-appointments-container');
                        if (container) {
                            // Trigger reload if function is in scope
                            if (typeof loadPendingAppointments === 'function') loadPendingAppointments();
                        }
                    }
                })
                .catch(err => {
            ...
            addMoreServiceBtn.addEventListener('click', function() {
                ...
                serviceEntry.innerHTML = `
                    ...
                    <div class="form-group hospital-form-group">
                        <label for="service_amount_${serviceCount}" class="hospital-label">
                            <span class="label-icon">💰</span>
                            Service Amount
                            <span class="required-mark">*</span>
                        </label>
                        <div class="input-wrapper">
                            <input type="number" id="service_amount_${serviceCount}" name="service_amount[]" class="hospital-input" step="0.01" min="0" placeholder="Enter amount in PHP" required>
                            <div class="input-border"></div>
                        </div>
                    </div>
